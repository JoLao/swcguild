...
public class ProductDaoImpl implements ProductDao {
    ...
    private static final String SQL_INSERT_PRODUCT
            = "insert into products (productName, productDesc, categoryId) "
            + " values (?, ?, ?)";
    ...
    private static final String SQL_UPDATE_PRODUCT
            = "update products set productName = ?, productDesc = ?, categoryId = ?"
            + " where productId = ?";
    ...
    public Product getProductById(int productId) {
        try {
            return jdbcTemplate.queryForObject(SQL_SELECT_PRODUCT, new ProductMapper(), productId);
