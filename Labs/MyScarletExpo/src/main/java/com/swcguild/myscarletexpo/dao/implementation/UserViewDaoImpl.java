...
public class UserViewDaoImpl implements UserViewDao {
    ...
    public static final String SQL_SELECT_ALL_USERS
            = "SELECT u.*, CASE WHEN au.authnum = 1 THEN 'Admin' "
            + "WHEN au.authnum = 2 THEN 'Manager' "
            + "WHEN au.authnum = 3 THEN 'Employee' "
            + "ELSE 'Customer' END AS authority "
            + "FROM users u "
            + "INNER JOIN ( "
            + "SELECT a.username, "
            + "MIN(CASE WHEN authority = 'admin' THEN 1 "
            + "WHEN authority = 'manager' THEN 2 "
            + "WHEN authority = 'employee' THEN 3 "
            + "ELSE 4 "
            + "END) as authnum "
            + "FROM authorities a "
