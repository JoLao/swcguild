                ...
                <div class="col-xs-6">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            ...
                            <sf:form class="form-horizontal" role="form" method="post" action="editProduct" modelAttribute="product">
                                    ...
                                    <div class="col-xs-9">
                                        <sf:select path="categoryId" class="form-control" id="productCategory" name="productCategory">
                                            <c:forEach var="category" items="${categories}">
                                                <c:set var="cSel" value=""/>                                                 
                                                <c:if test="${product.categoryId == category.categoryId}">
                                                    <c:set var="cSel" value="selected"/>
                                                </c:if>
                                            <option value="${category.categoryId}" ${cSel}>${category.categoryName}</option>
                                            </c:forEach>
                                        </sf:select>
                                    </div>
                                </div>
